...
const Cart = () => {
  ...
  return (
    <>
      <div
        className={`fixed block top-14 left-0 right-0 bottom-0 blur-effect-theme duration-500 w-full h-screen opacity-100 z-[600] ${ifCartState
      ...
      >
        ...
        >
          ...
          {cartItems?.length === 0 ? <CartEmpty onCartToggle={onCartToggle} /> : <div>
            <div className="flex items-start justify-start flex-col gap-y-7 lg:gap-y-5 overflow-y-scroll h-[81vh] scroll-smooth scroll-hidden pt-3 pb-20">
              {cartItems?.map((item, i) => (
                <CartItem key={i} item={item} />
              ))}
            </div>

            <div className="fixed bottom-0 pb-10 bg-white w-full px-5 py-2 grid items-center">
